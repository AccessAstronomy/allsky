    ...
    def allsky(self):
        ...
        if self.config["Time"]["Wait_Night"] == "True":
            while ephem.Date(datetime.datetime.utcnow()) <= self.Twilight_Sunset:
                ...
                sleep_trigger += 1
                self.status.clear_bit(0)  # Clear bit 0 to indicate daytime
                os.system(a)
        ...
        self.status.set_bit(0)  # Set bit 0 to indicate nighttime
        ...
        a = f"mv $HOME/enviro.dat {self.day_dir}/{self.day_string}-enviro.dat"
        os.system(a)
...
def main():
    slacker("Allsky starting up","wave","#228b22")
    #continuous = True
    ...
    slacker("End of Allsky, should restart later","sleeping","#222222")            
