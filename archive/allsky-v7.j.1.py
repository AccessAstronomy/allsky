...
gatech.lat, gatech.lon= '53.240463', '6.536159'
gatech.elevation=-2
...
while cc ==1:
    ...
    if x >= MEZsunrise and x <= MEZsunset:
        ...
        a='pkill -f gphoto2'
        os.system(a)
#        a='gphoto2 --set-config eosremoterelease=Immediate --wait-event=30s'
#        os.system(a) 
#        a='gphoto2 --set-config eosremoterelease="Press Full" --capture-image-and-download --filename="%Y-%m-%dT%H-%M-%S_001.cr2"'
#        a='gphoto2 --set-config eosremoterelease="Press Full" --wait-event-and-download=5s --filename="%Y-%m-%dT%H-%M-%S_001.cr2"'
        a='gphoto2 --wait-event=2s --set-config eosremoterelease=Immediate --wait-event=30s --set-config eosremoterelease="Press Full" --wait-event-and-download=5s --filename="%Y-%m-%dT%H-%M-%S_001.cr3"'
