...
gatech.lat, gatech.lon= '53.2439', '6.5333'
gatech.elevation=-2
...
while cc ==1:
    ...
    if x >= MEZsunrise and x <= MEZsunset:
        ...
        a='sudo gphoto2  --set-config eosremoterelease=Immediate --wait-event=30s --set-config eosremoterelease="Release Full" --wait-event-and-download=2s --filename=SQC_%Y-%m-%dT%H-%M-%S_001.cr2'
        os.system(a)      
